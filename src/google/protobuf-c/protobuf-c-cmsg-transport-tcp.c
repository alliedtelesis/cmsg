...
//This func can be used for ONEWAY_TCP as well
static int32_t
...
cmsg_transport_tcp_listen (cmsg_server* server)
{
  ...
  if (ret == -1)
  {
    DEBUG (" setsockopt failed with: %s\n", strerror(errno));
  ...
  DEBUG (" listening on tcp socket: %d\n", listening_socket);
...
cmsg_transport_tcp_client_recv (cmsg_client* client)
{
  ...
  if (nbytes == sizeof (cmsg_header_response))
  {
    ...
    if (nbytes == dyn_len)
    {
      ...
      if (message == NULL)
      {
        DEBUG (" error unpacking response message\n");
        return NULL;
      }
      return message;
    }
