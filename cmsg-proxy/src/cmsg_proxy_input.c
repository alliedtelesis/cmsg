...
{
    char *error_msg = NULL;
    bool data_expected = (strcmp (body_string, "") != 0);
    bool ret = false;

    if (data_expected && !data)
    {
        if (CMSG_PROXY_ASPRINTF (&error_msg, "Input expected but not provided.") < 0)
        {
            error_msg = NULL;
        }
    }
    else if (!data_expected && data)
    {
        if (CMSG_PROXY_ASPRINTF (&error_msg,
                                 "No JSON data expected for API, but JSON data input") < 0)
        {
            error_msg = NULL;
        }
    }
