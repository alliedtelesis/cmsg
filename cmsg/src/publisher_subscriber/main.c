 ...
 */
static gboolean
cmsg_psd_healthcheck_read (GIOChannel *source, GIOCondition condition, gpointer data)
{
    healthcheck_reply ();
    return TRUE;
}
 ...
 */
static gboolean
cmsg_psd_healthcheck_init (void *unused)
{
    int select_fd;
    ...
    if (healthcheck_init (&select_fd))
    {
        ...
        g_io_add_watch (healthcheck_channel, G_IO_IN, cmsg_psd_healthcheck_read, NULL);
...
main (int argc, char **argv)
{
    ...
    g_timeout_add_seconds (1, cmsg_psd_healthcheck_init, NULL);
