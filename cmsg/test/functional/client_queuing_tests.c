...
server_thread_process (void *arg)
{
    ...
    case CMSG_TRANSPORT_ONEWAY_TCP:
        server_transport = cmsg_transport_new (CMSG_TRANSPORT_ONEWAY_TCP);
...
run_client_queuing_tests (cmsg_queue_filter_type queue_type, bool all)
{
    ...
    create_server_and_wait (CMSG_TRANSPORT_ONEWAY_TCP);

    client = create_client (CMSG_TRANSPORT_ONEWAY_TCP);
